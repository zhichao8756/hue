...
class SQLIndexer(object):
  ...
  def create_table_from_a_file(self, source, destination, start_time=-1):
    ...
    if external or (load_data and table_format in ('parquet', 'orc', 'kudu')): # We'll use location to load data
      ...
      if split.scheme in ('', 'hdfs') and not hive_site.hiveserver2_impersonation_enabled() and not self.check_perm_for_hive(parent_path):
        ...
        self.fs.do_as_user(self.user, self.fs.chown, user_scratch_dir, self.user, 'hive')
  ...
  def check_perm_for_hive(self, path):
    try:
